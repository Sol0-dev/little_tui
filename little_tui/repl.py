...
class Repl:
    ...
    def _dispatch(self, line: str) -> bool:
        """Route one raw input line; returns True when the REPL should exit.

        Slash commands are single-line only: a multiline paste (embedded
        newlines) is always a normal prompt, even if it starts with ``/``.
        """
    ...
    def _resume_session(self, session_id: str) -> None:
        ...
        self.agent = self._build_agent()
        self.renderer.separator()
        self._show_history(self.history)
        self.renderer.separator()
        self.renderer.info(
    ...
    def _show_history(self, messages: list[dict[str, Any]]) -> None:
        """Render a replayed conversation so a resumed session shows its full chat."""
        for message in messages:
            role = message.get("role")
            if role == "user":
                self.renderer.user(message.get("content") or "")
            elif role == "assistant":
                ...
                for call in message.get("tool_calls") or []:
                    ...
                    print(
                        self.renderer._paint(
                            f"  ⚙ {name} {_compact_args(args)}", "dim"
                        )
                    )
            elif role == "tool":
                text = " ".join(str(message.get("content") or "").split())
                print(self.renderer._paint(f"  → {text[:120]}", "dim"))
    ...
    def _install_history(self) -> None:
        ...
        except Exception:  # noqa: BLE001 - older readline builds lack bracketed paste
            pass
        try:
