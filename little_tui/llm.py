"""OpenRouter chat client: streaming SSE, tool-call accumulation, retries.

Only ``requests`` is used. The response schema is OpenAI-compatible; OpenRouter
normalizes every provider to it. Streaming works for all models, and the final
SSE chunk carries ``usage`` (with cost) even when token deltas were streamed.
"""
...
def _error_from_response(resp: requests.Response) -> LLMError:
    """Parse the OpenRouter error envelope from a non-streaming failure."""
