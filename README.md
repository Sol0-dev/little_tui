...
current response. You can paste multi-line text (for example a code snippet or a
list of steps) into the prompt; it is sent as one task, not one per line.
...
- `/sessions <id>` resumes a conversation directly. The full conversation is
  shown again, and new turns keep appending to the same log.
...
| `LITTLE_TUI_MAX_HISTORY_TOKENS` | Max past-conversation tokens sent per request (default 2000) |
...
Long conversations: every request resends the recent history. To keep requests
small, only the newest turns that fit within `LITTLE_TUI_MAX_HISTORY_TOKENS` are
sent (old turns are dropped whole, so tool calls always stay paired with their
results). Lower it on heavily rate-limited free tiers; raise it to keep more
early context.
...
installer sets this model automatically when you pick Groq. Groq also counts
`max_tokens` against that per-minute budget, so a large `max_tokens` means fewer
requests per minute. If you keep hitting rate limits, lower `max_tokens` (for
example 2048) or `LITTLE_TUI_MAX_HISTORY_TOKENS`. Rate-limited requests are
retried with the provider's suggested delay before they fail.
...
| `Request too large ... TPM` (Groq) | Free-tier per-minute token limit. Lower `max_tokens` (for example 2048) in the config file, or wait a minute. Rate-limited requests retry automatically. |
